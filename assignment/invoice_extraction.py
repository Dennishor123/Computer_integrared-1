...
def extract_invoice_info(image_path):
    ...
    invoice_number = re.search(r"Invoice Number:\s*(\w+)", extracted_text)
    if invoice_number:
        invoice_number = invoice_number.group(1)

    # Extract date using regular expressions
    date = re.search(r"Date:\s*(\d{4}-\d{2}-\d{2})", extracted_text)
    if date:
        date = date.group(1)

    # Extract amount using regular expressions
    amount = re.search(r"Amount:\s*(\$\d+\.\d{2})", extracted_text)
    if amount:
...
# Example usage
folder_path = "/home/dennis/computer_integrated"
